...
def GetMispAttributes(misp_url, misp_key, misp_verifycert):
    ...
    attributes = misp.search(
        controller="attributes", to_ids=1, pythonify=True, publish_timestamp="89d"
    )
    ...
    # print totals
    print(
        f"[+] Total MISP indicators: {ipv4_length + ipv6_length + domain_length + url_length + hostname_length + sha256_length + sha1_length + md5_length}"
    )
    ...
    for ioc_type in iocs_to_upload:
        for ioc in iocs_to_upload[ioc_type]:
            response = falcon.indicator_create(
                ...
                retrodetects=True,
                description="IOC from MISP Database",
